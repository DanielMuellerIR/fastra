...
#   4. Die Tests `cmdw` und `newwindow` brauchen ECHTEN Fenster-Fokus. macOS 26 verweigert
...
ALL_TESTS=(windows newwindow multisearch findbar fields tabswitch highlight markdown jump ghosttext replaceall pilldrop navmatch colsel gutterdim search project localization git gitactions filemodes selsearch wildcard openscope contrast cmdw)
# Fensterlose Tests — laufen auch bei gesperrtem Bildschirm aussagekräftig.
WINDOWLESS_TESTS=(search project localization git gitactions filemodes selsearch wildcard openscope)
...
for t in "${TESTS[@]}"; do
    ...
    if [[ "$t" == "cmdw" || "$t" == "newwindow" ]]; then
        # cmdw und newwindow brauchen echten Fokus → via `open` starten (LaunchServices)
        # und von außen aktivieren.

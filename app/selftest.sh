...
ALL_TESTS=(windows newwindow welcomenew multisearch findbar fields tabswitch highlight highlight4d previewrender xpath markdown markdownjump markdownappearance jump ghosttext replaceall pilldrop navmatch colsel gutterdim sidebarheader sidebarfilter filediff searchmark help mdassist search project localization updates git gitactions filemodes selsearch wildcard openscope contrast cmdw)
...
for t in "${TESTS[@]}"; do
    ...
    if [[ "$t" == "cmdw" || "$t" == "newwindow" || "$t" == "welcomenew" ]]; then
        # cmdw, newwindow und welcomenew brauchen echten Fokus → via `open`

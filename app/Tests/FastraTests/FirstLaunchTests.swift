...
@MainActor
func workspace_firstLaunch_startsWithPlaceholder() {
...
func workspace_secondLaunch_startsEmpty() {
    ...
    // Folgestart legt den unberührten Start-Tab an; sein Titel ist der
    // lokalisierte Basisname (de: „Ohne Titel", sonst „Untitled").
    #expect(second.tabs.first?.title == Workspace.untitledBaseName)
    #expect(second.tabs.first?.isPristineScratch == true)

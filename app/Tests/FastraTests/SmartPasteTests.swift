...
func markdownFromClipboard_inheritedStderrDoesNotBlock() throws {
    ...
    let started = Date()
    let result = SmartPaste.markdownFromClipboard(mdClipURL: stub, timeout: 8)
    ...
    // Puffer hinzu. Fünf Sekunden bleiben trotzdem klar unter den acht
    // Sekunden des geerbten Descriptors: Ein Rückfall auf EOF-Warten wird
    // weiter sicher rot, normale Scheduler-Latenz nicht.
    #expect(elapsed < 5.0, "Fehlerpfad wartete \(elapsed) Sekunden auf Pipe-EOF")
}

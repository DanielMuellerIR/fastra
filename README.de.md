...
Fastra ist ein nativer macOS-Editor für Suchen & Ersetzen: so einfach wie ein
Texteditor, so mächtig wie reguläre Ausdrücke — und man sieht immer exakt,
was sich ändern wird, bevor ein einziges Zeichen angefasst wird.
...
Und wenn eine Aufgabe wirklich die volle Kraft braucht: RegEx-Modus
einschalten — mit Token-Highlighting, kuratierten Vorlagen und geführten
Capture Groups.
...
- **`*`-Platzhalter-Suche** mit Capture-Semantik — ohne RegEx-Kenntnisse.
...
- **Markdown-Vorschau** und Smart-Paste (formatierte Zwischenablage → Markdown).
...
Maschinerie — und entsprechender Lernkurve. Fastra deckt die Alltagsfälle ab,
ohne dass man ein Handbuch braucht.
